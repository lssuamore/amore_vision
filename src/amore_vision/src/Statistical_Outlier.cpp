...
{
	pcl::PCLPointCloud2 stat_outlier;
	pcl::StatisticalOutlierRemoval<pcl::PCLPointCloud2> out;
	out.setInputCloud (cloud);
	out.setMeanK(1);
	out.setNegative(false);
	out.setStddevMulThresh(5);
	out.filter (stat_outlier);
	pub_stat.publish(stat_outlier);
}
...
{
	// Initialize ROS
	ros::init (argc, argv, "Vision/Statistical_Outlier.cpp");
	ros::NodeHandle nh;
	get_params();
	...
	pub_stat = nh.advertise<sensor_msgs::PointCloud2> ("/vision/filtered/statistical_outlier", 10);
	// Spin
	ros::spin ();
}
